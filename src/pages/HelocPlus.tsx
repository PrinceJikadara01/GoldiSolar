...
export const HelocPlus = () => {
  ...
  useEffect(() => {
    ...
    const initPlayer = () => {
      playerRef.current = new (window as any).YT.Player("youtube-player-plus", {
        videoId: "OOHMWIbWu_s",
        playerVars: {
          autoplay: 1,
          mute: 1,
    ...
    <div className="bg-[#050505] min-h-screen text-white pt-24 font-sans selection:bg-goldi-green selection:text-black">
      ...
      <section className="h-[60vh] flex flex-col items-center justify-center text-center px-4">
        <motion.p
          ...
          className="text-goldi-green font-medium tracking-widest uppercase mb-4"
        >
          Introducing
        </motion.p>
          <h1 className="text-5xl md:text-7xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-zinc-500 tracking-tight mb-6">
              HELOC{" "} PLUS
            </h1>
      ...
      <section
        ref={videoSectionRef}
        className="relative w-full max-w-6xl mx-auto px-4 py-16 scroll-mt-24"
      >
        <div className="relative aspect-video rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(255,255,255,0.05)] border border-zinc-800 bg-black">
          <div className="absolute top-1/2 left-1/2 w-[200%] h-[200%] md:w-[115%] md:h-[120%] -translate-x-1/2 -translate-y-1/2 pointer-events-none">
            <div id="youtube-player-plus" className="w-full h-full"></div>
          </div>
